...
export default class Vehicles extends Component<{}, State> {
	...
	async getVehiclePosition() {
		...
		try {
			...
			showNotification('info', 'Araç konumu haritada işaretlendi');
		} catch (err) {
			showNotification('error', 'Önce bu aracı sipariş edin');
		}
	}
	...
	async spawnVehicle() {
		...
		try {
			...
			showNotification('info', 'Aracınız yakında teslim edilecek');
		} catch (err) {
	...
	async despawnVehicle() {
		...
		try {
			...
			showNotification('info', 'Kısa süre içinde çekici gelecek.');
		} catch (err) {
			if (err.msg) showNotification('error', err.msg);
		}
	}
			...
			<div className="vehicles">
				{selectedVehicle ? (
					...
					<div className="vehicles_main">
						<Title className="vehicles_main-title">Araçlarınız</Title>
