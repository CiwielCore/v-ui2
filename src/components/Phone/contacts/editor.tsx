...
export default function ContactEditor({ contact, save, remove, close }: Props) {
	return (
		<div className="contacts_editor">
			<Navigation
				close={{ title: 'İptal', onClick: close }}
				action={{ title: 'Tamam', form: 'contact-editor' }}
			/>
			...
			>
				<Form id="contact-editor">
					<Group>
						<Field type="text" name="firstName" placeholder="İsim" component={Input} />
						<Field type="text" name="lastName" placeholder="Soyisim" component={Input} />
						<Field
							type="text"
							name="phone"
			...
			{contact && (
				<Button onClick={remove} color="red">Kişiyi Sil</Button>
			)}
		</div>
	);
}
