		...
		return (
			<div className="business_main">
				...
				<div className="business_info">
					<BusinessInfoItem title="Sahibi">{owner || 'Yok'}</BusinessInfoItem>
					<BusinessInfoItem title="Günlük gelir">
						{prettify.price(income)}
					</BusinessInfoItem>
					<BusinessInfoItem title="Günlük maliyet">
						{prettify.price(tax)}
					</BusinessInfoItem>
					<BusinessInfoItem title={isOwner ? 'Devlet satışı' : 'Fiyat'}>
						{prettify.price(price)}
					</BusinessInfoItem>

					{isOwner && (
						<div className="business_info-owned">
							<BusinessInfoItem title="Ödenen gün">{paid.toString()}</BusinessInfoItem>
							<BusinessInfoItem title="Kalan süre">{`${hour.toString().padStart(2, '0')}:${min.toString().padStart(2, '0')}`}</BusinessInfoItem>
						</div>
					)}
				</div>
			</div>
