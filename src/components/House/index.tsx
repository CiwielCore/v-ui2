...
export default class House extends Component<Props, State> {
	...
	async trade() {
		...
		try {
			...
			showNotification(
				...
				isOwner
					? 'Başarıyla satıldı!'
					: 'Tebrikler, satın aldınız! Hizmet ödemeleri için bankaya gidin.'
			);
	...
	async toggleLock() {
		try {
			...
			showNotification('error', 'Erişim yok!');
	...
	async toEnter() {
		try {
			...
			showNotification('error', 'Kapı kilitli');
		}
	}
